...
void leftQual(){
  ...
  chassis.pid_drive_set(31.661, 85); //drive till in front of match load
  chassis.pid_wait();  
  matchLoad.extend(); 
  ...
  pros::delay(300); //wait for matchload to extend
  ...
  pros::delay(550); //wait a bit before retracting matchload
  matchLoad.retract(); //retract match load 
  chassis.pid_wait();
  chassis.pid_drive_set(-20, 40);

  tripleStateLongGoal(); //prepare to score long goal

  fullIntake(); //score long goal
  pros::delay(1600); //wait for balls to be scored
  stopIntake();

  chassis.pid_drive_set(10, 115); //move out from long goal
  chassis.pid_wait();
  tripleStateStore(); //prepare to intake more balls
  ...
  chassis.pid_turn_set(get_heading(-23.8, 21.729, -12.28, 9.194) + 183, 115); //turn back to mid goal
  pros::delay(650);

  chassis.pid_drive_set(-1 * (get_distance(-23.8, 21.729, -12.28, 9.194) + 1.5), 115); //drive into mid goal backwards
  tripleStateMidGoal(); //prepare to score mid goal
  //slowOuttake(); //outtake a bit to free up balls
  pros::delay(600);
...
void rightQual(){
  ...
  chassis.pid_drive_set(get_distance(-45.833, -13.946, -28.707, -21)+2, 55);
  ...
  fullIntake(); //intake matchload ballsD
...
//   chassis.pid_turn_set(get_heading(-34.6, 13.946,-22.257, 23.414), 105, true); //turn to face balls
//   pros::delay(500);
 
//   chassis.pid_drive_set(get_distance(-34.6, 14.917,-22.257, 24.5), 65);
//   fullIntake(); //intake balls
//   pros::delay(800);
//   setIntake(50,-50);
//  //intake balls
...
//   chassis.pid_turn_set(get_heading(-22.257, 24.5,-8.5, 11.18)+180, 90);//turn to mid goal
//   pros::delay(700);

//   chassis.pid_drive_set(-1*(get_distance(-22.257, 24.5,-8.5, 11.18)), 75); //drive into mid goal
//   pros::delay(500);
...
//   chassis.pid_drive_set(get_distance(-8.5,11.18,-47.077, 45.259), 95); //drive up to 1st matchload
//   pros::delay(1100);
...
//   pros::delay(700);
//   chassis.pid_drive_set(get_distance(-47.077, 45.259,-60,44.659), 105); //drive into matchload
//   fullIntake();
//   pros::delay(600);
//   chassis.pid_drive_set(1, 30);
//   pros::delay(1500);
//   chassis.pid_drive_set(-get_distance(-47.077, 45.259,-60,45.259), 105); //drive out of matchload
//   pros::delay(500);

//   chassis.pid_turn_set(get_heading(-47.077, 45.259, -37.243, 55.834) + 180, 105); //turn to face point beyond long goal
//   pros::delay(500);

//   chassis.pid_drive_set(-get_distance(-47.077, 45.259, -37.243, 55.834), 80); //drive past long goal
//   pros::delay(800);
...
//   chassis.pid_drive_set(-get_distance(-37.243, 55.834, 36.312, 55.834), 80); //drive to other side of field
//   pros::delay(400);
//   pros::delay(1800);
...
//   pros::delay(1500);  
//   stopIntake();
//   chassis.pid_drive_set(-get_distance(29.5, 46.259,59,46.25), 75);
//   pros::delay(800);
//   tripleStateLongGoal();
//   fullIntake();
//   pros::delay(1500);//score
//   tripleStateStore();
//   stopIntake();
  chassis.pid_drive_set(get_distance(28.36, 46.259,47.451,46.25), 95);
  pros::delay(600);
  chassis.pid_turn_set(get_heading(47.451,46.25,62.698,23.449), 75); //turn to be straight
  pros::delay(500);
  chassis.pid_drive_set(get_distance(47.451,46.25,62.698,23.449)-15, 75); //turn to be straight
  pros::delay(800);
  // chassis.pid_swing_set(ez::LEFT_SWING, 180, 90);
  // pros::delay(500);
  chassis.pid_swing_set(ez::LEFT_SWING, 180, 90, 50);
  pros::delay(1000);
  ...
  // chassis.pid_drive_set(-70.43, 127); //drive to other side of field
  // chassis.pid_wait();

  // chassis.pid_turn_set(0, 105); //turn perpendicular to long goal
  // pros::delay(500);

  // chassis.pid_drive_set(-11.6, 105); //drive to setup long goal
  // chassis.pid_wait();
  ...
  // // 1st long goal score
  // chassis.pid_drive_set(-9, 60); //drive into long goal
  // hood.extend(); //extend hood to prepare for scoring
  // pros::delay(700);
  ...
  // chassis.pid_drive_set(34, 50); //move out from long goal, into matchload
  // matchLoad.extend(); 
  // pros::delay(1200);
  // match_load_procedure(2, 0.5);
  ...
  // chassis.pid_drive_set(-27, 75); //move backwards into long goal
  // pros::delay(500); //wait a bit before stopping intake
  // intakeState = 0;
  // setIntakeMotors();
  // hood.extend(); //extend hood
  // chassis.pid_wait();
  ...
  // hood.retract();
  // pros::delay(200);
  // chassis.pid_drive_set(2, 105); //drive out and back in for hood descore
  // pros::delay(500);
  // chassis.pid_drive_set(-2, 40);
  // pros::delay(500);
  ...
  // chassis.pid_turn_set(180, 105); //turn to other side of field
  // pros::delay(500);

  // chassis.pid_drive_set(93.352, 85); //drive to set up right side matchload
  // pros::delay(2000);

  // chassis.pid_turn_set(90, 105); //turn to matchload
  // pros::delay(500);

  // chassis.pid_drive_set(23.591, 65); //drive into matchload
  // matchLoad.extend();
  // pros::delay(1000);
  // match_load_procedure(2, 0.5);
  
  // chassis.pid_drive_set(-12.5, 80); //drive out of matchload
  // pros::delay(600);
  // intakeState = 0;
  // setIntakeMotors();

  // chassis.pid_turn_set(get_heading(46.175, -47.5, 31.371,-60) + 180, 105); //turn back to right alley lane
  // pros::delay(500);

  // chassis.pid_drive_set(get_distance(46.175, -47.5, 31.371,-60) * -1, 75); //drive to alley lane
  // matchLoad.retract();
  // pros::delay(1000);

  // chassis.pid_turn_set(90, 60); //turn to face parralel to long goal
  // pros::delay(500);

  // chassis.pid_drive_set(-70.43, 127); //drive back to other side of field
  // chassis.pid_wait();
  ...
  // // 3rd long goal score
  // chassis.pid_drive_set(-9.5, 60); //drive into long goal
  // hood.extend(); //extend hood to prepare for scoring
  // pros::delay(700);

  // setIntake(-127, 127); //score balls into long goal
  // pros::delay(2700); //wait for balls to be scored
  // setIntake(0,0); //stop intaking
  // hood.retract(); //retract hood to slightly push ball further into long goal
  // pros::delay(200);
  ...
  // chassis.pid_drive_set(-31, 75); //move backwards into long goal
  // pros::delay(500); //wait a bit before stopping intake
  // intakeState = 0;
  // setIntakeMotors();
  // hood.extend(); //extend hood
  ...
  // chassis.pid_drive_set(22.5, 105); //drive out, get close to wall
  // pros::delay(1000);

  // chassis.pid_turn_set(328, 105); //turn to face parking barrier
  // pros::delay(500);

  // chassis.pid_drive_set(27, 30); //drive forward slightly into the wall to get closer
  // pros::delay(1400);
  // chassis.pid_turn_set(350, 80);
  // pros::delay(500);
  ...
  // intakeState = 2; //intake to clear balls in parking zone
  // setIntakeMotors();
...
//   wing.extend();
//   tripleStateStore();
//   //chassis.pid_drive_set(-4, 100); //nudge alliance
//   //pros::delay(300);

//   chassis.pid_drive_set(32.4, 85); //drive till in front of match load
//   //chassis.pid_drive_set((32.4+19), 85); //drive till in front of match load
...
//   chassis.pid_drive_set(-33, 115); //move backwards into long goal
//   pros::delay(525); //wait a bit before retracting matchload
//   matchLoad.retract(); //retract match load 
//   chassis.pid_wait();

//   tripleStateLongGoal(); //extend hood to prepare for scoring
