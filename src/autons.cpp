...
void leftQual(){
  ...
  chassis.pid_drive_set(32.661, 70+speedInc*2); //drive till in front of match load
  chassis.pid_wait();
  ...
  chassis.pid_drive_set(11, 55+speedInc); //drive into matchload
  ...
  pros::delay(550); //wait for balls to be intaken

  chassis.pid_drive_set(-27.961, 75+speedInc); //move backwards into long goal
  ...
  chassis.pid_drive_set(-1.5, 20); //keep moving while soring

  pros::delay(1150); //wait for balls to be scored +150
  setIntake(0,0); //stop intaking

  chassis.pid_drive_set(10, 60+speedInc); //move out from long goal
  chassis.pid_wait();
  hood.retract(); //retract hood after moving out of long goal

  chassis.pid_turn_set(get_heading(-40.021, 46.798, -23.3, 21.729), 105); //turn to face next set of balls
  chassis.pid_wait();

  chassis.pid_drive_set(get_distance(-40.021, 46.798, -23.3, 21.729), 45); //drive to next set of balls
  ...
  chassis.pid_turn_set(get_heading(-23.3, 21.729, -9.351, 41.17), 105); //turn to balls under long goal
  chassis.pid_wait();

  chassis.pid_drive_set(get_distance(-23.3, 21.729, -9.951, 41.17), 50); // get the balls under long goal
  intakeState = 4;
  ...
  chassis.pid_drive_set(get_distance(-23.3, 21.729, -9.951, 41.17) * -1, 75+speedInc); //drive back to set up for mid goal
  chassis.pid_wait();
  setIntake(0, 0); //stop intaking

  chassis.pid_turn_set(get_heading(-23.3, 21.729, -12.28, 9.194) + 180, 105); //turn back to mid goal
  pros::delay(500);

  chassis.pid_drive_set(-1 * get_distance(-23.3, 21.729, -12.28, 9.194), 65+speedInc*2); //drive into mid goal backwards
  hood.extend();
  intakeState = 3;
  setIntakeMotors();
  pros::delay(600);
  ...
  chassis.pid_drive_set(get_distance(-12.28, 9.194, -34.485, 35.2)+.5, 75+speedInc); //drive out of mid goal
  chassis.pid_wait();
  
  chassis.pid_turn_set(93, 103); //set up for wing descore
  chassis.pid_wait();
  pros::delay(100);

  chassis.pid_drive_set(15, 70+speedInc*3);
  chassis.pid_wait();
}
...
void rightQual(){
  ...
  chassis.pid_drive_set(10.677, 75+speedInc); //set up for middle balls
  pros::delay(350);

  chassis.pid_turn_set(get_heading(-35.227, -14.105, -21.536, -23.272), 105+turnInc); //turn to face balls
  pros::delay(450);
  intakeState = 4; //start intaking balls
  setIntakeMotors();

  chassis.pid_drive_set(get_distance(-35.227, -14.105, -21.536, -23.272), 55); // move into balls to intake
  chassis.pid_wait();

  chassis.pid_turn_set(get_heading(-21.536, -23.272, -43.327, -49.605), 60+turnInc); //turn to matchload setup
  chassis.pid_wait();
  intakeState = 0;
  setIntakeMotors(); //stop intaking

  chassis.pid_drive_set(get_distance(-21.536, -23.272, -43.327, -49.605), 75+speedInc); //drive to matchload setup
  chassis.pid_wait();
  ...
  intakeState = 4;
  setIntakeMotors();
  chassis.pid_drive_set(16, 50+speedInc); //drive into matchload while intaking
  chassis.pid_wait();
  pros::delay(200); //keep intaking for a bit after driving in

  chassis.pid_drive_set(-30.69, 80+speedInc); //drive backwards into long goal
  pros::delay(400);
  hood.extend();
  pros::delay(50);
  ...
  chassis.pid_drive_set(-20, 50+speedInc); //keep wheels driving back to help score

  setIntake(-127, 127); //score balls into long goal
  pros::delay(2250); //wait for balls to be scored
  setIntake(0,0); //stop intaking

  chassis.pid_drive_set(8.6777, 60+speedInc); //drive out to set up wing
  chassis.pid_wait();
  hood.retract();

  chassis.pid_turn_set(0, 105+turnInc); //turn to set up wing
  chassis.pid_wait();

  chassis.pid_drive_set(11.5, 75+speedInc); //drive to alley lane
  chassis.pid_wait();

  chassis.pid_turn_set(270, 105+turnInc); //turn to face direction for wing
  chassis.pid_wait();

  wing.retract(); //put wing down

  chassis.pid_drive_set(-27, 85+speedInc); //drive into alley and wing
  chassis.pid_wait();
...
void leftElim(){
  ...
  chassis.pid_drive_set(get_distance(-35.227, 14.105, -21.536, 23.272), 45); // move into balls to intake
  chassis.pid_wait();
  ...
  chassis.pid_wait();
  chassis.pid_drive_set(10.0, 75); //drive to alley lane
  chassis.pid_wait();
