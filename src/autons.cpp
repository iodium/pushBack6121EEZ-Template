 ...
 */
void rightLowGoal(){
  chassis.drive_angle_set(180_deg); //sets initial heading to facing 0 degrees
  wing.extend();
  
  chassis.pid_drive_set(31.6, 80); //drive till in front of match load
  pros::delay(1000);
  ...
  chassis.pid_drive_set(12.5, 65); //drive into matchload
  fullIntake();
  pros::delay(1000); //wait for balls to be intaken
  ...
  chassis.pid_turn_set(10.5, 105); // turn to face next set of balls while at long goal
  pros::delay(800);

  chassis.pid_drive_set(17.1, 60); //move forward into next set of balls
  hood.retract();
  fullIntake();
  pros::delay(800);
  
  chassis.pid_turn_set(48, 115); // facing mid low goal
  pros::delay(600);

  chassis.pid_drive_set(get_distance(-23.8, -19.729, -13.5, -13.5) - 1, 65); //drive into low goal
  stopIntake();
  pros::delay(600);

  fullOuttake(); //outtake to score low goal
  pros::delay(1750);
  stopIntake(); //stop scoring
  
  chassis.pid_turn_set(45, 105); //turn to face back to wing setup
  pros::delay(400);

  chassis.pid_drive_set(-28, 70); //drive backwards to set up wing push
  pros::delay(900);

  chassis.pid_turn_set(270, 110); //turn to set up wing push
  pros::delay(600);

  wing.retract();

  chassis.pid_drive_set(-25, 127); //drive backwards for wing push
  chassis.pid_wait();
}
...
void rightQual(){
  ...
  chassis.pid_turn_set(get_heading(-45.833, -13.946, -21, -23.5), 127, true); //turn to face balls
  pros::delay(600);

  chassis.pid_drive_set(get_distance(-45.833, -13.946, -21, -23.5), 60);
  fullIntake(); //intake balls  
  pros::delay(800);
  matchLoad.extend(); //extend matchload
  pros::delay(200);
  chassis.pid_turn_set(get_heading(-21, -23.5, -43.327, -45.5), 110); //turn to matchload setup
  pros::delay(500);

  // chassis.pid_drive_set(get_distance(-28.707, -21, -43.327, -45.5), 90); //drive to matchload setup
  // pros::delay(800);

  chassis.pid_drive_set(31, 90); //drive to matchload setup
  pros::delay(800);
  ...
  chassis.pid_drive_set(22, 65); //drive into matchload while intaking
  pros::delay(1350);

  chassis.pid_drive_set(-29, 110); //drive backwards into long goal
  pros::delay(600);
  ...
  chassis.pid_drive_set(14.2, 127); //drive to alley lane
  pros::delay(600);
...
void leftElim(){
  ...
  chassis.pid_turn_set(get_heading(-45.833, 13.946, -22, 23.5), 127, true); //turn to face balls
  pros::delay(600);

  chassis.pid_drive_set(get_distance(-45.833, 13.946, -22, 23), 65);
  fullIntake(); //intake balls  
  pros::delay(800);
  matchLoad.extend(); //extend matchload
  pros::delay(200);
  chassis.pid_turn_set(get_heading(-22, 23, -43.327, 45.5), 110); //turn to matchload setup
  pros::delay(500);

  chassis.pid_drive_set(32, 90); //drive to matchload setup
  pros::delay(900);
  ...
  fullIntake(); //intake matchload balls
  chassis.pid_drive_set(23.5, 70); //drive into matchload while intaking
  pros::delay(800);
  ...
  matchLoad.retract(); //retract matchload 
  chassis.pid_drive_set(-20, 40); //keep wheels driving back to help score
  ...
  chassis.pid_drive_set(10.5, 127); //drive to alley lane
  pros::delay(600);
  ...
  chassis.pid_drive_set(-24, 127); //drive into alley and wing
...
void soloAWP() {
  ...
  chassis.pid_drive_set(13, 75); //drive into matchload
  fullIntake();
  pros::delay(850); //wait for balls to be intaken
  ...
  hood.extend();
  fullIntake();
  pros::delay(700); //wait for balls to be scored
  stopIntake();
  ...
  chassis.pid_turn_set(180, 105); // turn to face next balls
  pros::delay(500);
  ...
  chassis.pid_turn_set(get_heading(-23, -22, -41, -48.3), 90);
  pros::delay(500);

  chassis.pid_drive_set(get_distance(-23, -22, -41, -48.3) - 2, 105);
  pros::delay(800);

  chassis.pid_turn_set(270, 105);

  chassis.pid_drive_set(-13, 80); //move backwards into long goal
  pros::delay(700); //wait a bit before retracting matchload
  

  chassis.pid_drive_set(-40, 30);

  hood.extend();
  fullIntake();
  pros::delay(1000); //wait for balls to be scored
  chassis.pid_drive_set(0, 1);
  stopIntake();

  chassis.pid_drive_set(28.1, 75); //drive into matchload
  pros::delay(200);
  hood.retract();
  fullIntake();
  ...
  chassis.pid_turn_set(47, 105);
  pros::delay(500);

  chassis.pid_drive_set(get_distance(-48.148, -47.114, -23, -22) + 1, 80); //move forward into next set of balls
  hood.retract();
  pros::delay(800);

  chassis.pid_drive_set(get_distance(-23.8, -19.729, -13.5, -13.5) + 8, 65); //drive into low goal
  stopIntake();
  pros::delay(450);

  fullOuttake(); //score low goal
}
void soloAWPYellow() {
